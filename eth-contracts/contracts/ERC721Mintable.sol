...
contract Ownable {
    ...
    */
    ...
    /**
    * @dev Modifier that requires an Address to have be valid 
    */
    modifier onlyValidAddress(address anAddress)
    {
        require(anAddress != address(0), "Address is not valid");
        _;
    }
    ...
    */     
    ...
    /**
    * @dev Transfer Ownership of a contract
    */      
...
contract Pausable is Ownable {
    ...
    /**
    * @dev Modifier that requires the pause flag to be on paused
    */
    modifier requirePaused()
    {
        require(_paused, "Require contract to be on pause");
        _;
    }

    /**
    * @dev Modifier that requires the pause flag to be not on paused
    */
    modifier requireWhenNotPaused()
    {
        require(!_paused, "Require contract not be on pause");
        _;
    }


    event Paused(address sender);
    event notInPaused(address sender);
    ...
    */
    ...
    /**
    * @dev public setter using the inherited onlyOwner modifier 
    */   
    function setPaused(bool status) public onlyOwner {
        _paused = status;

        if (_paused) emit Paused(msg.sender);
        else emit notInPaused(msg.sender);
    }
}
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(to != _tokenOwner[tokenId], "Approver cannot be the owner of the token");
        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(msg.sender == getOwner() || isApprovedForAll(_tokenOwner[tokenId], msg.sender));
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(_tokenOwner[tokenId] == from, "only owner can transfer ownership");
        // TODO: require token is being transfered to valid address
        require(!to.isContract(), 'transferring address is not a valid address');
        // TODO: clear approval
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    function getter() external view returns(string memory, string memory, string memory){
...
contract CustomERC721Token is ERC721Metadata("RealEstateToken", "RET", "https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/") {
